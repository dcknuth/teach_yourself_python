# Python Lesson 7
My daughter had quit working with me before this lesson (can’t remember exactly when), but I just kept going as a refresher for myself. This lesson is about regular expressions in Python
```
# Regular expressions
...
```
I never did get around to the phone number finding script. If you want to do it, it shouldn’t be too much more than the examples included above.

Note: RegEx is tricky. So tricky that it is probably best to do sensitive or simple things without it using the more straight forward string features. But if the stakes are low, it can be very quick and efficient, which fits well with Python
